...
chroma_collection_name = "erezept"
...
def load_directories_into_store(directories, chroma_db_file):
    documents = []
    for directory in directories:
        reader = SimpleDirectoryReader(input_dir=directory)
        documents += reader.load_data()
    
    # Initialisiere den ChromaVectorStore
    vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
    
    # Erstelle einen Index aus den Dokumenten und speichere ihn im ChromaVectorStore
...
if reload_directories:
    index = load_directories_into_store(directories, chroma_db_file)
else:
...
user_proxy = UserProxyAgent(
    name="User_Proxy",
    system_message="Du bist der Benutzer. Dein Ziel ist es, nützlich und hilfreiche Informationen und Unterstützung von den Agenten zu erhalten. Das Ergebnis soll möglichst fehlerfrei sein. Wenn die Agenten von einem Aspekt keine Kenntnisse haben, sollen die das auch so mitteilen.",
    llm_config={"config_list": autogen_az_balanced_configlist},
